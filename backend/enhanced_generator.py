...
class EnhancedNumberGenerator:
    ...
    def initialize_models(self, df: pd.DataFrame, use_ensemble: bool = True):
        """
        初始化所有数学模型
        ...
        # 提取高级特征
        print("提取高级特征...")
        df_enhanced = self.feature_extractor.extract_all_features(df)
        
        if use_ensemble:
            # 使用集成预测器
            print("初始化集成预测器...")
            self.ensemble_predictor.train(df_enhanced)
            ...
            self.markov_models = {
                ...
                'block_order1': MarkovChainModel(order=1, state_type='block'),
                'block_order2': MarkovChainModel(order=2, state_type='block'),
                'pattern_order1': MarkovChainModel(order=1, state_type='pattern'),
            }
        ...
        # 运行大数据分析
        print("运行大数据分析...")
        self.big_data_insights = self.big_data_analyzer.generate_insights(df_enhanced)
    ...
    def generate_two_rounds(
        ...
        # 第一轮生成
        print("生成第一轮号码...")
        first_round = self.generate_enhanced_numbers(
            count=count,
            rules=rules,
            front_blocks=front_blocks,
            back_blocks=back_blocks,
